...
function Module.Beta(From, To, T, Distance)
	local Points = workspace.Nodes:GetChildren()
	function search(name)
		for _, v in pairs(Points) do
			if v.Name == name then
	...
	local NetworkId = Networks:Add(Net)
	local Pos = NetPosition.new(From, To, T, NetworkId)
	local InDistance = NetNav:StepDistance(Pos, Distance, 1)
	workspace.Marker2.Position = NetNav:GetVecPos(Pos)
	workspace.Marker.Position = NetNav:GetVecPos(InDistance)
	Networks:Remove(NetworkId)
	return InDistance
end
